...
async def create_food_entry(entry: FoodEntry):
    ...
    current_total = sum(
        existing.total_calories 
        for existing in food_log.values()
        if existing.user.id == entry.user.id 
    )
    if current_total + entry.total_calories > entry.user.max_daily_calories:
